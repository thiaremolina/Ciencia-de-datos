...
def mediana(vals):
    ...
    if n % 2 == 0:
        # Si el número de datos es par, promediamos los dos valores centrales:
...
def varianza(vals, tipo ='Poblacional'):
    ...
    """
     ...
     if n < 2:
         return 0
     media = promedio(vals)
     # 2. calcular la sumatoria de las  diferencias al cuadrado:
     diferencias_cuadrado = sum([(x - media) ** 2 for x in vals])
     # 3. Determinar el denominador según el tipo:
     if tipo =='Muestral': 
         denominador = n-1 
     else:
         denominador = n
...
def desviacion_mediana_absoluta(vals):  
    ...
    desviaciones = [abs(x - mediana) for x in vals] #abs() devuelve el valor absoluto de un número
...
def covarianza(vals_x,vals_y):
    ...
    m2 = promedio(vals_y)
     ...
     # 3. Calcular la suma de los productos de las diferencias:
    sumatoria = 0 
    for i in range(n):
        dif_x = vals_x[i] - m1
        dif_y = vals_y[i] - m2
